...
def runFromSavedState (file="state.sav"):
    ...
    contTime = sim.cfg.duration - time
    if sim.rank == 0: print('\nRunning simulation for %s ms... up to %s'%(contTime,sim.cfg.duration))
    sim.pc.psolve(contTime)
    ...
    if sim.rank==0:
        print('  Done; run time = %0.2f s; real-time ratio: %0.2f.' %
            (sim.timingData['runTime'], contTime/1000/sim.timingData['runTime']))
