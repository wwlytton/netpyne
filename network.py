...
class Network(object):
    ...
    def connectCells(self):
        ...
        for connParam in self.params['connParams']:  # for each conn rule or parameter set
            ...
            for condKey,condValue in connParam['preTags'].items():  # Find subset of cells that match presyn criteria
                if condKey == 'yfrac':
                    preCells = {gid: tags for (gid,tags) in preCells.items() if condValue[0] <= tags[condKey] < condValue[1]}  # dict with pre cell tags
                    prePops = {}
                else:
                    preCells = {gid: tags for (gid,tags) in preCells.items() if tags[condKey] in condValue}  # dict with pre cell tags
                    prePops = {i: tags for (i,tags) in prePops.items() if (condKey in tags) and (tags[condKey] in condValue)}
            ...
            for condKey,condValue in connParam['postTags'].items():  # Find subset of cells that match postsyn criteria
                if condKey == 'yfrac':
                    postCells = {gid: cell for (gid,cell) in postCells.items() if condValue[0] <= cell.tags[condKey] < condValue[1]}  # dict with post Cell objects}  # dict with pre cell tags
                else:
                    postCells = {gid: cell for (gid,cell) in postCells.items() if cell.tags[condKey] in condValue}  # dict with post Cell objects
   ...
   ###############################################################################
    ...
    def fullConn(self, preCells, postCells, connParam):
        ...
        for postCellGid, postCell in postCells.items():  # for each postsyn cell
            for preCellGid,preCellTags in preCells.items():  # for each presyn cell
    ...
    def randConn(self, preCells, postCells, connParam):
        ...
        if all (k in connParam for k in ('delayMean', 'delayVar')):  # generate list of delays based on mean and variance
            ...
            delay = connParam['delay']  # fixed delay
        for postCellGid, postCell in postCells.items():  # for each postsyn cell
            preCellGids = list(preCells.keys())
    ...
    def probConn(self, preCells, postCells, connParam):
        ...
        for postCell in postCells.values():
            ...
            for i,preInd in enumerate(preInds):
                if list(preCells)[preInd] == postCell.gid: break
                params = {'preGid': list(preCells)[preInd],
                'sec': connParam['sec'],
                'synReceptor': connParam['synReceptor'],
                'weight': weights[i],
                'delay': delays[i],
